...
const PATH = "/solutions/filmmakers";
const TITLE = "AI Previsualization & Storyboard Tool for Filmmakers";
const DESCRIPTION =
  "Use MuseForge for shot previsualization, animatics, and storyboard generation on indie film projects — no storyboard artist required.";

const trail = (locale) => [
  { name: t(locale, "nav_home"), path: withLocale("/", locale) },
  { name: t(locale, "sol_filmmakers"), path: withLocale(PATH, locale) },
];
...
export default function FilmmakersPage({ params: { locale } }) {
